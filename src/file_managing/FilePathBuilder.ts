...
export abstract class FilePathBuilder {
    ...
    public static buildSplitFinalDatasetFolderPath(gen: number): string {
		if (gen === 0) {
			return SPLIT_FINAL_DATASETS_DIR;
		} else {
	...
	public static buildSplitFinalDatasetPathByFileName (gen: number, fileName: string): string {
		return FilePathBuilder.buildSplitFinalDatasetFolderPath(gen) + '/' + fileName;
	}
