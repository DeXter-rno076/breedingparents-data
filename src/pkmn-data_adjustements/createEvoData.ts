...
import { PkmnEvolutionLink } from '../types';
import Logger from '../Logger';

const bot = new Bot('shouldntbeneeded', 'thisshouldntbeneeded', 'https://www.pokewiki.de/api.php', true);
const evoDataMap = new Map<string, PkmnEvolutionLink>();
...
export default async function createEvoData() {
    ...
    const evoListArticleContent = await bot.getWikitext('Liste von Entwicklungen', 'Liste der Entwicklungslinien');
...
function handleTableRow(row: string) {
    ...
    const cellRegex = /\|\s*\[\[Datei:.+?\]\]\s*<br \/>\s*Nr\.\d{3,4}\s*(?<pkmn>.+)/;
    ...
    if (evos.length < 1) {
        /*multiple table rows have intendedly no evos (e. g. table head) 
		Logger.elog(
...
function addEvoObjs(evos: string[]) {
    ...
    const stage1Obj: PkmnEvolutionLink = {
        pre: '',
        name: evos[0],
        post: evos[1] || '',
    };
    ...
    if (evos.length >= 2) {
        const stage2Obj: PkmnEvolutionLink = {
            pre: evos[0],
            name: evos[1],
            post: evos[2] || '',
        };
    ...
    if (evos.length >= 3) {
        const stage3Obj: PkmnEvolutionLink = {
            pre: evos[1],
            name: evos[2],
            post: '',
        };
...
function saveMap() {
    ...
    fs.writeFileSync('data/in/evos.json', JSON.stringify(objForm));
}
...
function getPkmnName(tableCellPkmnName: string): string {
    ...
    let cleanName = tableCellPkmnName.replace('[[', '').replace(']]', '');
    if (cleanName.includes('[') || cleanName.includes(']')) {
        Logger.elog('getPkmnName: extracted pkmn name contains [ or ] ' + tableCellPkmnName);
    }

    if (cleanName.includes('|')) {
        const linkText = cleanName.split('|')[1];
        cleanName = linkText;
    }
...
function applyManualEvosData() {
    ...
    const manualEvosData = JSON.parse(fs.readFileSync('data/in/evosManual.json', { encoding: 'utf-8' })) as {
